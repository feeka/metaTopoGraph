...
struct ExtractionOptions {
    ...
    uint64_t max_bubble_depth  = 200;    // max edge depth per branch in bubble search
                                         // (SNP bubbles at k=21 need ~k+1=22 edges min)
    double   mode_prominence   = 0.10;   // min fraction of global max to count as a mode
};
