...
struct NodeFeatures {
    uint64_t n_prominent_jumps;      // transitions (i->j) where min(mul_i,mul_j) <= error_threshold
    ...
    double   error_threshold;        // Bayesian m*: mult <= this is more likely erroneous than real
};
